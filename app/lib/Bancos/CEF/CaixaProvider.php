...
class CaixaProvider
{
    ...
    public function __construct()
    {
        ...
        $this->identificadorOrigem =  $_SERVER['REMOTE_ADDR'];
    }
    ...
    public function consulta(BoletoCEF $boleto)
    {
    ...
    public function incluir(BoletoCEF $boleto)
    {
        $hashAutenticacao = $this->generatorHash(
            $boleto->getCodigoBeneficiario(),
            $boleto->getCnpj(),
      ...
      $documento= strlen($boleto->getCpf()) ;
          
          if($documento==11){
            $Pessoa=[ 'CPF' => $boleto->getCpf(),
            'NOME' => $boleto->getNome()];
            
          }else{
            
            $Pessoa=[ 'CNPJ' =>$boleto->getCpf(),
            'RAZAO_SOCIAL' => $boleto->getNome()];
    ...
    public function alterar(BoletoCEF $boleto)
    {
        $hashAutenticacao = $this->generatorHash(
            $boleto->getCodigoBeneficiario(),
            $boleto->getCnpj(),
    ...
    public function baixa(BoletoCEF $boleto)
    {
        $hashAutenticacao = $this->generatorHash(
            $boleto->getCodigoBeneficiario(),
            $boleto->getCnpj(),
